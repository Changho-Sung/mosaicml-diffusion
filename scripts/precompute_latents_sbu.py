...
from diffusion.datasets.sbu.transforms import LargestCenterSquare


class StreamingSBUDataset(StreamingDataset):
    """Implementation of the SBU dataset as a streaming dataset except with metadata.

    Args:
        streams (Sequence[Stream], optional): One or more Streams to stream/cache samples from. StreamingSBUDataset
    ...
    def __getitem__(self, index):
        ...
        if self.transform is None:
            img = Image.open(BytesIO(sample['jpg']).content)
            if img.mode != 'RGB':
                img = img.convert('RGB')
...
def build_streaming_sbu_dataloader(
    ...
    **dataloader_kwargs,
    ...
    """Builds a streaming SBU dataloader returning multiple image sizes.
...
def main(args: Namespace) -> None:
    """Add latents to SBU Captions dataset.
